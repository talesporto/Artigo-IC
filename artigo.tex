...
\title{A mídia de massa segundo os apocalípticos analisando o episódio ``O tarado Homer'' dos Simpsons}
...
As análises serão feitas a partir de um ponto de vista apocalíptico, conforme os conceitos da mídia de massa apresentados por Umberto Eco, com os quais é possível criticar a manipulação da informação modificando a cultura de massa~\cite{eco}.

Apesar de dirigir-se a um público heterogênio, a mídia de massa difunde globalmente uma cultura do tipo homegênio, destruindo as características culturais próprias de cada grupo étnico. Ela tende a impor símbolos e mitos de fácil universalidade, criando ``tipos'' prontamente reconhecíveis~\cite{eco}.  

Nos Simpsons, isso é exemplificado pelas sátiras que ridicularizam problemas sociais existentes criando imagens sócio-culturais exageradas, mostrando de forma irônica como a mídia impõe símbolos de maneira fácil. No episódio, isso ficou claro ao criar o estereótipo dos solteiros escoceses onde todos têm como \textit{hobby} filmar casais namorando dentro de carros, ou seja, um estereótipo de pervertidos. 

Além do episódio análisado, podemos ver esses símbolos também em outros episódios. Um episódio bem famoso no Brasil, é \textbf{O Feitiço de Lisa}, onde a família dos Simpsons visitam o Brasil. O episódio mostra o Rio de Janeiro com macacos e ratos nas ruas e com uma população sexualmente agressiva. No episódio, Homer é sequestrado por um taxista, Bart é atacado por pivetes, e os programas infantis tem apresentadoras provocantes~\cite{personagensSimpsons}. Ou seja, definindo a cidade como um lugar onde so há violência, sexo e sujeira.

Apresentando de forma sucinta a notícia, as mídias de massa capturam o nível superficial da nossa atenção. No episódio é mostrado um fato que ocorre com uma certa frequência nas mídias de massa. Se trata da manipulação da informação para que a mesma condiza com as opiniões do gerador da informação, e também com os seus interesses. Também é mostrado no episódio, e fica claro que os receptores da informação acreditaram na primeira história que ouviram e não buscaram se aprofundar e obter novas fontes para averiguar a veracidade da história. O que gerou um trantorno enorme para a vítima da calunia que teve que provar sua inocência, mesmo as acusações sendo infundadas e inconfiáveis.

A mídia de massa é colocada dentro de um circuito comercial produzindo e vendendo todo o tipo de produto buscando atingir o maior público possível. Como esse tipo de mídia está sujeita à “lei da oferta e da procura” cria se a desconfiança de que as notícias publicadas pelos mesmo são verdadeiras ou não. Não são raros casos em que se criam notícias para que se venda revistas, jornais e outros meios que transmitem esse tipo de informação e a partir dessa falsa ``acusação'' gira-se todo um mercado em volta onde a disputa de interesses, sejam esses políticos ou financeiros, prevalece~\cite{eco}. 

Apesar da desconfiança gerada nesse tipo de mídia a massa que quer consumir esse tipo de informação não se questiona sobre a veracidade das informações e acredita ``cegamente'' na informação que lhe foi entregue.
